...
func (worker Worker) Start(ctx context.Context, rabbitmqURL string) error {
	...
	log.Printf("Schedule generation worker consuming queue %s", queue.Name)
	for {
		select {
		case <-ctx.Done():
			return ctx.Err()
		case delivery, ok := <-deliveries:
			if !ok {
				return fmt.Errorf("generation queue deliveries closed")
			}
			if err := worker.handleDelivery(ctx, delivery); err != nil {
				log.Printf("generation job failed: %v", err)
...
func (worker Worker) publishJob(ctx context.Context, jobID string) {
	...
	if err != nil {
		log.Printf("load schedule generation notification job: %v", err)
		return
	}
